...
#define DISTANCE_THRESHOLD_CM 120
...
uint32_t HCSR04_Read_Distance_cm(void)
{
  ...
  /*
   * Measure HIGH duration of ECHO signal
   */
  __HAL_TIM_SET_COUNTER(&htim2, 0);
  ...
  while (HAL_GPIO_ReadPin(HCSR04_ECHO_PORT, HCSR04_ECHO_PIN) == GPIO_PIN_SET)
  {
    if ((HAL_GetTick() - timeout_start) > 30)
    {
      return 999; // Echo timeout
    }
  }

  pulse_time = __HAL_TIM_GET_COUNTER(&htim2);
...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  uint32_t distance = HCSR04_Read_Distance_cm();

	  if (distance == 999)
	  {
	    /*
	     * Sensor error / no echo.
	     * Blink slowly so we know the sensor reading failed.
	     */
	  ...
	  {
	    /*
	     * Object is closer than threshold.
	     * Blink fast.
	     */
	    HAL_GPIO_TogglePin(LED_PORT, LED_PIN);
	    HAL_Delay(100);
	  ...
	  {
	    /*
	     * Object is farther than threshold.
	     * LED OFF.
	     */
